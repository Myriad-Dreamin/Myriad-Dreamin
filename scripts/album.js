...
const updateReadme = () => {
    ...
    const albumCovers = [albums[0]].map(album => `<img src="${album.cover}" alt="${album.title}" width="480">`).join('|');
...
const updateAlbums = () => {
    ...
    const albumCovers = albums.map(album => `|<img src="${album.cover}" alt="${album.title}" width="480">|`);
